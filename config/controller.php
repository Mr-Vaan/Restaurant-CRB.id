<?php 
	...
	class Resto extends koneksi{
		...
		public function koneksi($data_kd, $data_kd2, $data_kd3,)
	    {
	        global $con;
	        $sql   = "SELECT kd_resto, nama_resto, alamat_resto, kd_kota, kd_kategori, kd_user FROM resto WHERE kd_resto = '$data_kd' AND kd_kota = '$data_kd2' AND kd_kategori = '$data_kd3'";
	        $query = mysqli_query($con, $sql);
	        $data  = [];
	        while ($bigData = mysqli_fetch_assoc($query)) {
	            $data[] = $bigData;
		...
		public function insert($table, $values, $redirect){
			...
			// 	$response = [
			// 		'response' => 'positive',
			// 		'alert' => 'Data Antrian berhasil ditambahkan',
			// 		// 'redirect' => $redirect
			// 		header("location: $redirect")
			// 	];
			// }else{
